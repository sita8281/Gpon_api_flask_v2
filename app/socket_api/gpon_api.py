...
def gpon_connection(type_gpon):
    ...
    return TelnetGponExec(login="gpon_int", password="admin123", host=(ip, 33))
...
def onu_list(gpon, slot, port):
    ...
    try:
        for i in normalize(data):
            ...
            if onu_id.isdigit():
                lst.append({"id": onu_id, "sn": onu_sn, "name": "", "port": port, "slot": slot, "state": onu_state})
                continue
...
def mac_onu_port(gpon, slot, port, onu_id):
    """
